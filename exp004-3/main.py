...
from models import FullyConvNet, FullyConvResNet, UNet, UNetBidirection, UNetBidirection2, UNetBidirection3, UNetBidirection4
...
def train_unsupervised(args, model, m_kernel, m_dict, reverse_m_dict):
    ...
    for epoch in range(args.train_epoch):
        ...
        if torch.cuda.is_available():
            ...
            ones = ones.cuda()
        im_pred, im_pred_f, m_mask_f, disappear_f, attn_f, im_pred_b, m_mask_b, disappear_b, attn_b = model(im_input_f, im_input_b, ones)
...
def main():
    ...
    model = UNetBidirection4(height, width, channel, num_inputs, len(m_dict), m_range, m_kernel)
    if torch.cuda.is_available():
